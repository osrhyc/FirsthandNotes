...
SKIP_RE = re.compile(
    ...
    r"|Contents|Copyright|COPYRIGHT|About the Author)", re.I)
...
def plan_units(nodes, depth=0):
    ...
    for n in nodes:
        ...
        for c in n["children"]:
            cur.append(c)
            if sum(x["chars"] for x in cur) >= TARGET:
                runs.append(cur)
                cur = []
        if cur:
            # 尾组明显不足目标的一半就并进上一组（仍在本章内，不跨章）
            if runs and sum(x["chars"] for x in cur) < TARGET // 2:
                runs[-1].extend(cur)
            else:
                runs.append(cur)
